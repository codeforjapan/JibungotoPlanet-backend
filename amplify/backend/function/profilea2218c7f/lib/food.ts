...
const estimateFood = async (
  ...
  const foodIntakeFactor = await dynamodb.query({
  ...
  if (foodAnswer.foodDirectWasteFactorKey && foodAnswer.foodLeftoverFactorKey) {
    const foodDirectWasteFactor =  await dynamodb.query({
      TableName: parameterTableName,
      KeyConditions: {
        category: {
          ComparisonOperator: 'EQ',
    ...
    const foodLeftoverFactor = await dynamodb.query({
      TableName: parameterTableName,
      KeyConditions: {
        category: {
          ComparisonOperator: 'EQ',
    ...
    const foodWastRatio = await dynamodb.query({
      TableName: parameterTableName,
      KeyConditions: {
        category: {
          ComparisonOperator: 'EQ',
    ...
    const leftoverRatio  = foodWastRatio.Items.find(item => item.key === 'leftover-per-food-waste')
    const directWasteRatio = foodWastRatio.Items.find(item => item.key === 'direct-waste-per-food-waste')
    const foodWasteRatio = foodWastRatio.Items.find(item => item.key === 'food-waste-per-food')
    ...
    const foodPurchaseAmountConsideringFoodLossRatio =
      (1+foodLossAverageRatio*foodWasteRatio.value)/(1+foodWasteRatio.value)

    estimationAmount.rice.value = estimationAmount.rice.value * foodIntakeFactor.Items[0]?.value * foodPurchaseAmountConsideringFoodLossRatio
    estimationAmount['bread-flour'].value = estimationAmount['bread-flour'].value * foodIntakeFactor.Items[0]?.value * foodPurchaseAmountConsideringFoodLossRatio
    estimationAmount.noodle.value = estimationAmount.noodle.value * foodIntakeFactor.Items[0]?.value * foodPurchaseAmountConsideringFoodLossRatio
    estimationAmount.potatoes.value = estimationAmount.potatoes.value * foodIntakeFactor.Items[0]?.value * foodPurchaseAmountConsideringFoodLossRatio
    estimationAmount.vegetables.value = estimationAmount.vegetables.value * foodIntakeFactor.Items[0]?.value * foodPurchaseAmountConsideringFoodLossRatio
    estimationAmount['processed-vegetables'].value = estimationAmount['processed-vegetables'].value * foodIntakeFactor.Items[0]?.value * foodPurchaseAmountConsideringFoodLossRatio
    estimationAmount.beans.value = estimationAmount.beans.value * foodIntakeFactor.Items[0]?.value * foodPurchaseAmountConsideringFoodLossRatio
    estimationAmount.fruits.value = estimationAmount.fruits.value * foodIntakeFactor.Items[0]?.value * foodPurchaseAmountConsideringFoodLossRatio
    estimationAmount.oil.value = estimationAmount.oil.value * foodIntakeFactor.Items[0]?.value * foodPurchaseAmountConsideringFoodLossRatio
    estimationAmount.seasoning.value = estimationAmount.seasoning.value * foodIntakeFactor.Items[0]?.value * foodPurchaseAmountConsideringFoodLossRatio
    estimationAmount['ready-meal'].value = estimationAmount['ready-meal'].value * foodIntakeFactor.Items[0]?.value * foodPurchaseAmountConsideringFoodLossRatio
    ...
    if(foodAnswer.dairyFoodFactorKey) {
      const dairyFoodFactor =  await dynamodb.query({
        TableName: parameterTableName,
        KeyConditions: {
          category: {
            ComparisonOperator: 'EQ',
      ...
      estimationAmount.milk.value = estimationAmount.milk.value * dairyFoodFactor.Items[0]?.value * foodPurchaseAmountConsideringFoodLossRatio
      estimationAmount['other-dairy'].value = estimationAmount['other-dairy'].value * dairyFoodFactor.Items[0]?.value * foodPurchaseAmountConsideringFoodLossRatio
      estimationAmount.eggs.value = estimationAmount.eggs.value * dairyFoodFactor.Items[0]?.value * foodPurchaseAmountConsideringFoodLossRatio
    ...
    if(foodAnswer.dishBeefFactorKey) {
      dishBeefFactor =  await dynamodb.query({
        TableName: parameterTableName,
        KeyConditions: {
          category: {
            ComparisonOperator: 'EQ',
      ...
      estimationAmount.beef.value = estimationAmount.beef.value * dishBeefFactor.Items[0]?.value * foodPurchaseAmountConsideringFoodLossRatio
    ...
    if(foodAnswer.dishPorkFactorKey) {
      dishPorkFactor = await dynamodb.query({
        TableName: parameterTableName,
        KeyConditions: {
          category: {
            ComparisonOperator: 'EQ',
      ...
      estimationAmount.pork.value = estimationAmount.pork.value * dishPorkFactor.Items[0]?.value * foodPurchaseAmountConsideringFoodLossRatio
      estimationAmount['other-meat'].value = estimationAmount['other-meat'].value * dishPorkFactor.Items[0]?.value * foodPurchaseAmountConsideringFoodLossRatio
    ...
    if(foodAnswer.dishChickenFactorKey) {
      dishChickenFactor = await dynamodb.query({
        TableName: parameterTableName,
        KeyConditions: {
          category: {
            ComparisonOperator: 'EQ',
      ...
      estimationAmount.chicken.value = estimationAmount.chicken.value * dishChickenFactor.Items[0]?.value * foodPurchaseAmountConsideringFoodLossRatio
    ...
    if(foodAnswer.dishSeafoodFactorKey) {
      const dishSeafoodFactor =  await dynamodb.query({
        TableName: parameterTableName,
        KeyConditions: {
          category: {
            ComparisonOperator: 'EQ',
      ...
      estimationAmount.fish.value = estimationAmount.pork.value * dishSeafoodFactor.Items[0]?.value * foodPurchaseAmountConsideringFoodLossRatio
      estimationAmount['processed-fish'].value = estimationAmount['processed-fish'].value * dishSeafoodFactor.Items[0]?.value * foodPurchaseAmountConsideringFoodLossRatio
    ...
    if(foodAnswer.alcoholFactorKey) {
      const alcoholFactor =  await dynamodb.query({
        TableName: parameterTableName,
        KeyConditions: {
          category: {
            ComparisonOperator: 'EQ',
      ...
      estimationAmount.alcohol.value = estimationAmount.alcohol.value * alcoholFactor.Items[0]?.value * foodPurchaseAmountConsideringFoodLossRatio
    ...
    if(foodAnswer.softDrinkSnackFactorKey) {
      const softDrinkSnackFactor =  await dynamodb.query({
        TableName: parameterTableName,
        KeyConditions: {
          category: {
            ComparisonOperator: 'EQ',
      ...
      estimationAmount['sweets-snack'].value = estimationAmount['sweets-snack'].value * softDrinkSnackFactor.Items[0]?.value * foodPurchaseAmountConsideringFoodLossRatio
      estimationAmount['coffee-tea'].value = estimationAmount['coffee-tea'].value * softDrinkSnackFactor.Items[0]?.value * foodPurchaseAmountConsideringFoodLossRatio
      estimationAmount['cold-drink'].value = estimationAmount['cold-drink'].value * softDrinkSnackFactor.Items[0]?.value * foodPurchaseAmountConsideringFoodLossRatio
